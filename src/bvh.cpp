	...
	struct get_tri_min_max{
		...
		thrust::tuple<vec3d,vec3d>
		 operator()(size_t i){
			...
			return thrust::make_tuple(bb.min_vec, bb.max_vec);
		 }
	...
	struct min_max{
		thrust::tuple<vec3d,vec3d>
			operator()(thrust::tuple<vec3d,vec3d> a,
				...
				const vec3d min_vec = cwisemin(
											   thrust::get<0>(a),
											   thrust::get<0>(b)
											  );
											  
				const vec3d max_vec = cwisemax(
											   thrust::get<1>(a),
											   thrust::get<1>(b)
											  );  
			
				return thrust::make_tuple(min_vec,max_vec); 
			}
	};
...
bvh_builder::bvh_builder(bvh_platform platform_,
			const mesh & m_):platform(platform_),
							 m(m_),
							 compare_obj_centroid(*this){
	root = NULL;
	num_tris = m.flist.size();
}
...
void bvh_builder::build(){
	...
	thrust::transform(remap_table.begin(),
					  remap_table.end(),
					  tri_bounds.begin(),
				  	  detail::compute_tri_aabb(m.vlist,m.flist)
	);
...
aabb bvh_builder::get_aabb_from(
	...
	//detail::get_tri_min_max taabb(m.vlist,m.flist);
...
aabb bvh_builder::get_object_aabb(size_t i)  const{
	...
	/*const vec3d va = m.vlist[m.flist[i].a];
	const vec3d vb = m.vlist[m.flist[i].b];
	const vec3d vc = m.vlist[m.flist[i].c];	
	const vec3d min_vec = cwisemin(cwisemin(va,vb),vc);
	const vec3d max_vec = cwisemax(cwisemax(va,vb),vc);  
...
struct __attribute__((__packed__)) large_node{
	...
	void operator=(const bvh_node *node){
		...
		x_min  = node->bb.min_vec.x;
		y_min  = node->bb.min_vec.y;
		z_min  = node->bb.min_vec.z;
		
		x_max  = node->bb.max_vec.x;
		y_max  = node->bb.max_vec.y;
		z_max  = node->bb.max_vec.z;		
	...
	if(oformat == SSV){
		...
		for(size_t i = 0; i < node_list.size(); i++ ){
			...
			if(!node->is_leaf()){
				...
				os <<  std::setprecision(15)  
				   ...
				   <<  inode->bb.min_vec.x << " " << inode->bb.min_vec.y << " " << inode->bb.min_vec.z << " " 
				   <<  inode->bb.max_vec.x << " " << inode->bb.max_vec.y << " " << inode->bb.max_vec.z << std::endl; 			   
			}else{
				leaf_node* lnode  = static_cast<leaf_node*>(node);
				 os <<  std::setprecision(15)  
					<<  1 << " " <<  lnode->start_idx << " " << lnode->end_idx   << " " 
					<<  lnode->bb.min_vec.x << " " << lnode->bb.min_vec.y << " " << lnode->bb.min_vec.z << " " 
					<<  lnode->bb.max_vec.x << " " << lnode->bb.max_vec.y << " " << lnode->bb.max_vec.z << std::endl; 			   
			}
		}
		...
		for(size_t i = 0; i < n_nodes;  i++){
			if(!node_list[i]->is_leaf()){
				...
				N1[i].x  = cnodes[0]->bb.min_vec.x; 
				N1[i].y  = cnodes[0]->bb.max_vec.x;
				N1[i].z  = cnodes[0]->bb.min_vec.y;
				N1[i].w  = cnodes[0]->bb.max_vec.y;

				N2[i].x  = cnodes[1]->bb.min_vec.x; 
				N2[i].y  = cnodes[1]->bb.max_vec.x;
				N2[i].z  = cnodes[1]->bb.min_vec.y;
				N2[i].w  = cnodes[1]->bb.max_vec.y;

				N3[i].x  = cnodes[0]->bb.min_vec.z; 
				N3[i].y  = cnodes[0]->bb.max_vec.z;
				N3[i].z  = cnodes[1]->bb.min_vec.z;
				N3[i].w  = cnodes[1]->bb.max_vec.z;
